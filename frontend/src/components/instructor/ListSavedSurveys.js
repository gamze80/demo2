...
const ListSavedSurveys = () => {
    ...
    const fetchSavedSurveys = async () => {
        ...
        try {
            const response = await fetch(
                `http://localhost:8080/api/v1/survey/get_distinct_survey_names_by_instructor_id_not_published?userId=${loggedInUser.userId}`,
                {
                    method: 'GET',
                    headers: {
    ...
    const handleSurveyClick = (survey) => {
        ...
        navigate("/mypage/instructor/editsurvey");
    };
            ...
            {savedSurveys.length > 0 ? (
                <ul>
                    {savedSurveys.map((survey) => (
                        <li key={survey}>
                            <button onClick={() => handleSurveyClick(survey)}>{survey}</button>
                        </li>
                    ))}
                </ul>
            ) : (
...
export default ListSavedSurveys;